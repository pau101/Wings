...
//	@Config.RangeDouble(min = 0.0D, max = 10.0D)
	...
	ConfigWingSettings(final ResourceLocation key) {
		this(key, 7, 0.001D, 2, 0.08D);
	}

	private ConfigWingSettings(final ResourceLocation key, final int requiredFlightSatiation, final double flyingExertion, final int requiredLandSatiation, final double landingExertion) {
		this.key = key;
