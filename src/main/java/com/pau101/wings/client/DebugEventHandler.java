...
public final class DebugEventHandler {
	...
	public static void init(ModelRegistryEvent event) {
		if (handler == null) {
			MinecraftForge.EVENT_BUS.register(handler = new Object() {
				...
				public void tick(TickEvent.ClientTickEvent event) {
					if (event.phase == TickEvent.Phase.END) {
						...
						if (world != null && (player == null || player.world != world)) {
							player = new EntityOtherPlayerMP(world, new GameProfile(UUID.fromString("617ab577-0da7-4d6a-a80d-0b516544369d"), "ModDeveloper")) {{
								getDataManager().set(PLAYER_MODEL_FLAG, (byte) 0xFF);
							}};
							player.setPosition(0, 62, 0);
							player.prevPosZ = -1;
							player.prevPosY = 63;
